...
import { Guild, MessageEmbed, StreamDispatcher, VoiceConnection } from 'discord.js';
...
declare module 'discord.js-commando' {
    interface CommandoClient {
        ...
        playFromQueue(server: Guild, connection: VoiceConnection, msg: CommandoMessage): void
...
export class GameClient extends CommandoClient {
    ...
    */
    ...
    playFromQueue(server: Guild, connection: VoiceConnection, msg: CommandoMessage) {
            ...
            dispatcher.on('end', (reason) => {
                if (queue.length <= 1) {
                    ...
                    if (reason !== 'cancelled') { 
                        ...
                        msg.channel.send(new MessageEmbed().setColor('#0303ff').setTitle("Trashmash").setDescription("All mashes played, thanks for playing!")); 
                    }
                } else {
                    queue.shift();
